...
$PYTHON "$ROOT/backtester/tools/compare_tsv_audit.py" "$ROOT/artifacts/backtests/golden/hf_daily_limits_audit_golden.tsv" "$ROOT/artifacts/backtests/reports/hf_daily_limits_report.tsv" || {
...
print('PropFriendly trace record count differs: golden=',s.get('records'),'got=',count)
exit(1)
